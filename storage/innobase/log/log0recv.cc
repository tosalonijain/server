...
public:
  ...
  {
    const byte * const recs= static_cast<const byte*>
  ...
  void free() const { recv_sys.free(this); }
};
...
create_block:
  ...
  block->page.access_time= ((block->page.access_time + (1U << 16)) &
                            (~0U << 16)) | static_cast<uint16_t>(free_offset);
  UNIV_MEM_INVALID(block->frame + free_offset - len, len);
...
inline bool page_recv_t::trim(lsn_t start_lsn)
{
  while (log.head)
  {
    ...
    static_cast<const log_phys_t*>(log.head)->free();
    log.head= next;
  }
  log.tail= nullptr;
  return true;
...
inline void page_recv_t::recs_t::clear()
{
  ...
  for (const log_rec_t *l= head; l; )
  {
    ...
    static_cast<const log_phys_t*>(l)->free();
    l= next;
  }
  head= tail= nullptr;
}
  ...
  default:
    if (log_phys_t *tail= static_cast<log_phys_t*>(recs.log.last()))
    {
      ...
      if (free_offset + len < srv_page_size &&
          tail->start_lsn == start_lsn &&
          tail->ends_at(block->frame + free_offset))
      {
...
eom_found:
  ...
  if (UNIV_UNLIKELY(recv_calc_lsn_on_data_add(start_lsn, l + 1 - log) >
                    scanned_lsn))
...
same_page:
    ...
    if (got_page_op)
    {
      ...
      case STORE_IF_EXISTS:
        ...
        /* fall through */
      case STORE_YES:
        add(page_id_t(space_id, page_no), start_lsn, recovered_lsn, recs,
...
# endif
  ...
  ut_ad(l == el);
  recovered_offset= l - buf;
  recovered_lsn= recv_calc_lsn_on_data_add(recovered_lsn, l - log);
  ut_ad(recovered_lsn <= scanned_lsn);
  goto loop;
}
			      ...
			      mlog_init_t::init* init = NULL)
{
	...
	for (const log_rec_t* recv : p->second.log) {
		...
		DBUG_LOG("ib_log", "apply " << l->start_lsn << ": "
			 << block->page.id);
